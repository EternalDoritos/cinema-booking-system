...
export const getStaticProps = async () => {
  ...
  const [movies, bookings] = await Promise.all([moviesRes.json(), bookingsRes.json()]);
      ...
      <div className="text-center flex flex-col m-4"><form>
        ...
        <label>
          ...
          <select value={selectedBooking} onChange={handleBookingChange}>
            ...
            {bookings?.filter(booking=>booking.movie == selectedMovie).map(booking => (
              <option key={booking._id} value={booking._id}>{booking.date} - {booking.time}</option>
