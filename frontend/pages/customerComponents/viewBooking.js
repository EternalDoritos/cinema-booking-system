...
const DisplayBooking = () => {
  ...
  const formatTime = (time) => {
    ...
    const formattedTime = new Date(`2000-01-01T${timeString.substring(0, 2)}:${timeString.substring(2, 4)}`);
  ...
  return (
    <div className="bg-gray-800 min-h-screen py-8">
      <div>
        ...
        <div className="mt-9">
          {seats.map((info) => (
            <div key={info.movieList._id} className="my-4 border border-gray-300 p-4 ml-6 mr-6">
                ...
                <div className="grid grid-col 2">
                  ...
                  <div>
                    {info.seating.map((seat, seatIndex) => (
                      <span key={seatIndex} className="text-white bg-blue-500 px-2 py-1 rounded ml-2">
                        {getSeatLabel(seat)}
